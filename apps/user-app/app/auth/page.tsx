...
export default function LoginSignup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res?.status === 201) {
      toast.success(
        "Welcome to EzyPay! Your account has been created successfully.",
      );
      router.push("/dashboard");
    } else if (res?.status === 200) {
      toast.success("You've successfully logged in to your EzyPay account.");
      router.push("/dashboard");
    } else {
      ...
      <div className="bg-white rounded-lg shadow-xl p-8 max-w-md w-full">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900">
            Welcome to <span className="text-blue-600 font-bold">EzyPay</span>
          </h1>
        ...
        <p className="mt-4 text-sm text-center text-gray-600">
          By continuing, you agree to EzyPay's Terms of Service and Privacy
          Policy.
        </p>
      </div>
    </div>
